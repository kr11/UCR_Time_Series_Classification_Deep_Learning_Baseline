...
class PositionwiseFeedForward():
    def __init__(self, d_hid, d_inner_hid, dropout=0.1, use_norm=True):
    ...
    def __call__(self, x):
        ...
        output = Add()([output, x])
        return self.layer_norm(output) if self.use_norm else output


class EncoderLayer():
    def __init__(self, d_model, d_inner_hid, n_head, d_k, d_v, dropout=0.1, att_use_norm=True, pos_use_norm=True):
        self.self_att_layer = MultiHeadAttention(n_head, d_model, d_k, d_v, dropout=dropout, use_norm=att_use_norm)
        self.pos_ffn_layer = PositionwiseFeedForward(d_model, d_inner_hid, dropout=dropout, use_norm=pos_use_norm)

    def __call__(self, enc_input, mask=None):
        output, slf_attn = self.self_att_layer(enc_input, enc_input, enc_input, mask=mask)
        output = self.pos_ffn_layer(output)
        # output = self.pos_ffn_layer(enc_input)
        return output, None
